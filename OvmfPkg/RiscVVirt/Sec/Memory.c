  ...
  )
{
  ...
  Prop = (INT32 *)FdtGetProperty (Fdt, Node, Name, &Len);
  if (Prop == NULL) {
    return Len;
  }
  ...
  )
{
  ...
  NumRsv = FdtNumRsv (FdtPointer);

  /* Look for an existing entry and add it to the efi mem map. */
  for (i = 0; i < NumRsv; i++) {
    if (FdtGetMemRsv (FdtPointer, i, &Addr, &Size) != 0) {
      continue;
    }
  ...
  if (Node >= 0) {
    ...
    FdtForEachSubnode (SubNode, FdtPointer, Node) {
      RegProp = (INT32 *)FdtGetProperty (FdtPointer, SubNode, "reg", &Len);
      ...
      if ((RegProp != 0) && (Len == ((NumAddrCells + NumSizeCells) * sizeof (INT32)))) {
        ...
        if (FdtGetProperty (FdtPointer, SubNode, "no-map", &Len)) {
          BuildMemoryAllocationHob (
            Addr,
            Size,
            EfiReservedMemoryType
  ...
  )
{
  ...
  for (Prev = 0; ; Prev = Node) {
    ...
    Type = (CHAR8 *)FdtGetProperty (FdtPointer, Node, "device_type", &Len);
    if (Type && (AsciiStrnCmp (Type, "memory", Len) == 0)) {
      // Get the 'reg' property of this node. For now, we will assume
      // two 8 byte quantities for base and size, respectively.
      RegProp = (UINT64 *)FdtGetProperty (FdtPointer, Node, "reg", &Len);
