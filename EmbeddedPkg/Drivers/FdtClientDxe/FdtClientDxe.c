  ...
  )
{
  ...
  *Prop = FdtGetProperty (mDeviceTreeBase, Node, PropertyName, &Len);
  if (*Prop == NULL) {
  ...
  )
{
  ...
  //
  NodeStatus = (CONST CHAR8 *)FdtGetProperty (mDeviceTreeBase, Node, "status", &Len);
  if (NodeStatus == NULL) {
    return TRUE;
  }
  ...
  )
{
  ...
  for (Prev = PrevNode; ; Prev = Next) {
    ...
    Type = (CONST CHAR8 *)FdtGetProperty (mDeviceTreeBase, Next, "compatible", &Len);
    if (Type == NULL) {
      continue;
    }
  ...
  )
{
  ...
  for (Prev = PrevNode; ; Prev = Next) {
    ...
    DeviceType = (CONST CHAR8 *)FdtGetProperty (mDeviceTreeBase, Next, "device_type", &Len);
