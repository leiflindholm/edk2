  ...
  )
{
  ...
  Prop = (INT32 *)FdtGetProperty (DeviceTreeBlob, 0, "#address-cells", &Length);
  if (Length == 4) {
    AddressCells = Fdt32ToCpu (*Prop);
  }

  Prop = (INT32 *)FdtGetProperty (DeviceTreeBlob, 0, "#size-cells", &Length);
  if (Length == 4) {
  ...
  // range listed.
  //
  Prop = (INT32 *)FdtGetProperty (DeviceTreeBlob, MemoryNode, "reg", &Length);
