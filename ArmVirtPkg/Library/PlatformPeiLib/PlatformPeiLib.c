  ...
  )
{
  ...
  *FdtHobData = (UINTN)NewBase;
  ...
  for (Prev = Depth = 0; ; Prev = Node) {
    ...
    Compatible = FdtGetProperty (Base, Node, "compatible", &Len);

    //
    // Iterate over the NULL-separated items in the compatible string
    //
    for (CompItem = Compatible; CompItem != NULL && CompItem < (CHAR8 *)Compatible + Len;
         CompItem += 1 + AsciiStrLen (CompItem))
    {
      if (FeaturePcdGet (PcdTpm2SupportEnabled) &&
          (AsciiStrCmp (CompItem, "tcg,tpm-tis-mmio") == 0))
      {
        RegProp = (UINT64 *)FdtGetProperty (Base, Node, "reg", &Len);
        ASSERT (Len == 8 || Len == 16);
        if (Len == 8) {
        ...
        if (Depth > 1) {
          ...
          //
          RangesProp = (UINT32 *)FdtGetProperty (Base, Parent, "ranges", &RangesLen);
