  ...
  )
{
  ...
  Prop = FdtGetProperty (Fdt, Node, "compatible", &PropLen);
  ...
  )
{
  ...
  Prop = FdtGetProperty (Fdt, Node, PropertyName, &Size);
  if ((Prop == NULL) || (Size < 0)) {
    return FALSE;
  }
  ...
  )
{
  ...
  while (TRUE) {
    ...
    Prop = FdtGetProperty (Fdt, Node, "interrupt-controller", &Size);
    if ((Prop != NULL) && (Size >= 0)) {
      ...
      *IntcNode = Node;
      ...
      PHandle = (UINT32 *)FdtGetProperty (Fdt, Node, "interrupt-parent", &Size);
  ...
  )
{
  ...
  Data = (UINT32 *)FdtGetProperty (Fdt, IntcNode, "#interrupt-cells", &Size);
