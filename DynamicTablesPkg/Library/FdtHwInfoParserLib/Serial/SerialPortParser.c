  ...
  )
{
  ...
  Data = (UINT8 *)FdtGetProperty (Fdt, SerialPortNode, "reg", &DataSize);
  if ((Data == NULL) ||
  ...
  Data = (UINT8 *)FdtGetProperty (Fdt, SerialPortNode, "interrupts", &DataSize);
  ...
  Data = (UINT8 *)FdtGetProperty (Fdt, SerialPortNode, "clock-frequency", &DataSize);
  if (Data != NULL) {
  ...
  if (FdtNodeIsCompatible (Fdt, SerialPortNode, &Serial16550CompatibleInfo)) {
    ...
    /* reg-io-width:
         ...
         UART.
    */
    Data = (UINT8 *)FdtGetProperty (Fdt, SerialPortNode, "reg-io-width", &DataSize);
    if (Data != NULL) {
      if (DataSize < sizeof (UINT32)) {
        // If error or not enough space.
        ASSERT (0);
  ...
  )
{
  ...
  Prop = (CHAR8 *)FdtGetProperty (Fdt, ChosenNode, "stdout-path", &PropSize);
